...
履歴ポップアップまたはメニューバーから選んだ履歴は、最後に操作していた通常アプリへ自動貼り付けされる。
自動貼り付けには、`システム設定 > プライバシーとセキュリティ > アクセシビリティ` で MacClipy を許可してもらう。
権限がない場合や貼り付け先を開けない場合でもコピーは完了しているため、案内に従って貼り付け先で `Command + V` を押してもらう。
