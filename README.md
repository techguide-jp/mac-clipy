...
- `Sources/MacClipy/Analytics`: 匿名インストールID、送信スケジュール、first-party APIクライアント
...
クリップボード履歴、お気に入りの内容や表示名、検索文字列、項目・フォルダのID、利用したアプリ名は送信しません。プロセス生存は常駐状態、パネル利用とお気に入りの追加・解除は明示的な利用として別の指標に分けます。
